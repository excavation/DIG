...
/*
    // CHECK IF STANDALONE
    QFileInfo check_standalone(QDir::homePath() + "/AppData/Roaming/Excavate/DIG-Client");
...
void GenerationPage::on_pushButton_8_clicked()
{
    ...
    if (dHashesPerSec < 1000)
    {
        //
#ifdef Q_OS_LINUX
        QString curtxt = QDir::homePath() + "/.Excavate/Hashes.sec";
#elif defined(Q_OS_MAC)
        QString curtxt = QDir::homePath() + QDir::toNativeSeparators("/Library/Application Support/Excavate/Hashes.sec");
#else // Windows
        QString curtxt = QDir::homePath() + "/AppData/Roaming/Excavate/Hashes.sec";
#endif
        QFile fileV(curtxt);
    ...
    else if (dHashesPerSec > 999)
    {
        //
#ifdef Q_OS_LINUX
        QString curtxt = QDir::homePath() + "/.Excavate/kHashes.sec";
#elif defined(Q_OS_MAC)
        QString curtxt = QDir::homePath() + QDir::toNativeSeparators("/Library/Application Support/Excavate/kHashes.sec");
#else //Windows
        QString curtxt = QDir::homePath() + "/AppData/Roaming/Excavate/kHashes.sec";
#endif
        QFile fileV(curtxt);
        if(!fileV.open(QIODevice::ReadOnly | QIODevice::Text))
    ...
    else if (dHashesPerSec > 999999)
    {
        //
#ifdef Q_OS_LINUX
        QString curtxt = QDir::homePath() + "/.Excavate/mHashes.sec";
#elif defined(Q_OS_MAC)
        QString curtxt = QDir::homePath() +  QDir::toNativeSeparators("/Library/Application Support/Excavate/mHashes.sec");
#else //Windows
        QString curtxt = QDir::homePath() + "/AppData/Roaming/Excavate/mHashes.sec";
#endif
        QFile fileV(curtxt);
        if(!fileV.open(QIODevice::ReadOnly | QIODevice::Text))
